...
int encrypt_aria_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv);
int decrypt_aria_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv);
int encrypt_aria_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key32, int ksize);
int decrypt_aria_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key32, int ksize);
...
{
    unsigned char	iv[_CRYPT_BLOCK_SIZE ] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	...
	if (is_pkcs7)
	{
		if (is_enc) { tlen = encrypt_aria_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, klen, iv); }
		else        { tlen = decrypt_aria_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, klen, iv); }
	}else
	{
		if (is_enc) { tlen = encrypt_aria_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey, klen); }
		else        { tlen = decrypt_aria_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey, klen); }
	}
...
static NCRYPT_ARIA_ROUNDS GetARIA_Rounds(int szKey)
{
	if (szKey == 16) return NCRYPT_ARIA_ROUND12;
	if (szKey == 24) return NCRYPT_ARIA_ROUND14;
	if (szKey == 32) return NCRYPT_ARIA_ROUND16;
	return NCRYPT_ARIA_ROUND_MAX;
}

int encrypt_aria_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv)
{
	unsigned char pBuf[_MAX_MSG_SIZE];
	NCRYPT_ALGO_ARIA_ARGS stARIA;
	NCODE_TYPE_BASE64_ARGS stBase64;
	int rtVal;

	memset(&stARIA, 0x00, sizeof(stARIA));
	stARIA.eRounds = GetARIA_Rounds(ksize);
	stARIA.stArgs.isEncrypt = 1;
	stARIA.stArgs.eAlgo = NCRYPT_ALGO_ARIA;
	stARIA.stArgs.ePadd = NCRYPT_PADD_PKCS;
	stARIA.stArgs.eMode = NCRYPT_MODE_CBC;
	stARIA.stArgs.pbIV  = iv;
	stARIA.stArgs.szIV  = _CRYPT_BLOCK_SIZE;
	stARIA.stArgs.pbKey = key32;
	stARIA.stArgs.szKey = ksize;
	stARIA.stArgs.pbInStream = sdata;
	stARIA.stArgs.szInStream = slen;
	stARIA.stArgs.pbOutBuf   = pBuf;
	stARIA.stArgs.szOutBuf   = _MAX_MSG_SIZE;

	if ((rtVal = Ncrypt(&stARIA, sizeof(stARIA))) < 0)
	{
		fprintf(stderr, "Ncrypt(encrypt_ARIA_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
	...
	{
		fprintf(stderr, "Encode(encrypt_ARIA_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
...
int decrypt_aria_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv)
{
	...
	stARIA.stArgs.pbKey = key32;
	stARIA.stArgs.szKey = ksize;
	stARIA.stArgs.pbInStream = pBuf;
	stARIA.stArgs.szInStream = rtVal;
	stARIA.stArgs.pbOutBuf   = tdata;
	stARIA.stArgs.szOutBuf   = _MAX_MSG_SIZE;
...
int encrypt_aria_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key32, int ksize)
{
	...
	stARIA.stArgs.isEncrypt = 1;
	stARIA.stArgs.eAlgo = NCRYPT_ALGO_ARIA;
	stARIA.stArgs.ePadd = NCRYPT_PADD_NULL;
	stARIA.stArgs.eMode = NCRYPT_MODE_ECB;
	stARIA.stArgs.pbKey = key32;
	stARIA.stArgs.szKey = ksize;
	stARIA.stArgs.pbInStream = sdata;
	stARIA.stArgs.szInStream = slen;
