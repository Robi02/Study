...
int encrypt_seed_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key16, unsigned char *iv);
int decrypt_seed_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key16, unsigned char *iv);
int encrypt_seed_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key16);
int decrypt_seed_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key16);
...
void main(int argc, char **argv)
{
    ...
    char pchar;
    char ekey[64] ,smsg[_MAX_MSG_SIZE] ,tmsg[_MAX_MSG_SIZE];
	...
	if (is_pkcs7)
	{
		if (is_enc) { tlen = encrypt_seed_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, iv); }
		else        { tlen = decrypt_seed_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, iv); }
	}else
	{
		if (is_enc) { tlen = encrypt_seed_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey); }
		else        { tlen = decrypt_seed_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey); }
	}
...
int encrypt_seed_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key16, unsigned char *iv)
{
	...
	NCRYPT_ALGO_SEED_ARGS stSEED;
	NCODE_TYPE_BASE64_ARGS stBase64;
	int rtVal;

	memset(&stSEED, 0x00, sizeof(stSEED));
	stSEED.stArgs.isEncrypt = 1;
	stSEED.stArgs.eAlgo = NCRYPT_ALGO_SEED;
	stSEED.stArgs.ePadd = NCRYPT_PADD_PKCS;
	stSEED.stArgs.eMode = NCRYPT_MODE_CBC;
	stSEED.stArgs.pbIV  = iv;
	stSEED.stArgs.szIV  = _CRYPT_BLOCK_SIZE;
	stSEED.stArgs.pbKey = key16;
	stSEED.stArgs.szKey = strlen(key16);
	stSEED.stArgs.pbInStream = sdata;
	stSEED.stArgs.szInStream = slen;
	stSEED.stArgs.pbOutBuf   = pBuf;
	stSEED.stArgs.szOutBuf   = _MAX_MSG_SIZE;

	if ((rtVal = Ncrypt(&stSEED, sizeof(stSEED))) < 0)
	{
		fprintf(stderr, "Ncrypt(encrypt_seed_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
	...
	{
		fprintf(stderr, "Encode(encrypt_seed_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
...
int decrypt_seed_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key16, unsigned char *iv)
{
	...
	stSEED.stArgs.pbKey = key16;
	stSEED.stArgs.szKey = strlen(key16);
	stSEED.stArgs.pbInStream = pBuf;
	stSEED.stArgs.szInStream = rtVal;
	stSEED.stArgs.pbOutBuf   = tdata;
	stSEED.stArgs.szOutBuf   = _MAX_MSG_SIZE;
...
int encrypt_seed_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key16)
{
	...
	stSEED.stArgs.isEncrypt = 1;
	stSEED.stArgs.eAlgo = NCRYPT_ALGO_SEED;
	stSEED.stArgs.ePadd = NCRYPT_PADD_NULL;
	stSEED.stArgs.eMode = NCRYPT_MODE_ECB;
	stSEED.stArgs.pbKey = key16;
	stSEED.stArgs.szKey = strlen(key16);
	stSEED.stArgs.pbInStream = sdata;
	stSEED.stArgs.szInStream = slen;
