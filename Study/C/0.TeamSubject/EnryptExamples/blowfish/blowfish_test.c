...
int encrypt_blowfish_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv);
int decrypt_blowfish_cbc_pkcs7(char *tdata ,char *sdata, int slen, unsigned char *key32, int ksize, unsigned char *iv);
int encrypt_blowfish_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key32, int ksize);
int decrypt_blowfish_ecb(char *tdata ,char *sdata, int slen, char pchar ,unsigned char *key32, int ksize);
...
void main(int argc, char **argv)
{
    ...
    if (0 == memcmp(argv[3] ,"H/" ,2))
    {
		...
		if (8 > klen || klen < 112) {printUsage(3, argv);}
	...
	{
		klen = strlen(argv[3]);
		if (4 > klen || klen > 56) {printUsage(3, argv);}
	...
	if (is_pkcs7)
	{
		if (is_enc) { tlen = encrypt_blowfish_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, klen, iv); }
		else        { tlen = decrypt_blowfish_cbc_pkcs7(tmsg ,smsg, slen, (unsigned char *)ekey, klen, iv); }
	}else
	{
		if (is_enc) { tlen = encrypt_blowfish_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey, klen); }
		else        { tlen = decrypt_blowfish_ecb(tmsg ,smsg, slen, pchar, (unsigned char *)ekey, klen); }
	}
...
unsigned char pBuf[_MAX_MSG_SIZE];
	NCRYPT_ALGO_BLOWFISH_ARGS stBLOWFISH;
	NCODE_TYPE_BASE64_ARGS stBase64;
	int rtVal;

	memset(&stBLOWFISH, 0x00, sizeof(stBLOWFISH));
	stBLOWFISH.stArgs.isEncrypt = 1;
	stBLOWFISH.stArgs.eAlgo = NCRYPT_ALGO_BLOWFISH;
	stBLOWFISH.stArgs.ePadd = NCRYPT_PADD_PKCS;
	stBLOWFISH.stArgs.eMode = NCRYPT_MODE_CBC;
	stBLOWFISH.stArgs.pbIV  = iv;
	stBLOWFISH.stArgs.szIV  = _CRYPT_BLOCK_SIZE;
	stBLOWFISH.stArgs.pbKey = key32;
	stBLOWFISH.stArgs.szKey = ksize;
	stBLOWFISH.stArgs.pbInStream = sdata;
	stBLOWFISH.stArgs.szInStream = slen;
	stBLOWFISH.stArgs.pbOutBuf   = pBuf;
	stBLOWFISH.stArgs.szOutBuf   = _MAX_MSG_SIZE;

	if ((rtVal = Ncrypt(&stBLOWFISH, sizeof(stBLOWFISH))) < 0)
	{
		fprintf(stderr, "Ncrypt(encrypt_BLOWFISH_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
	...
	{
		fprintf(stderr, "Encode(encrypt_BLOWFISH_cbc_pkcs7) error. (return:%d)\n", rtVal);
		return rtVal;
	}
...
unsigned char pBuf[_MAX_MSG_SIZE];
	...
	stBLOWFISH.stArgs.pbKey = key32;
	stBLOWFISH.stArgs.szKey = ksize;
	stBLOWFISH.stArgs.pbInStream = pBuf;
	stBLOWFISH.stArgs.szInStream = rtVal;
	stBLOWFISH.stArgs.pbOutBuf   = tdata;
	stBLOWFISH.stArgs.szOutBuf   = _MAX_MSG_SIZE;
...
unsigned char pBuf[_MAX_MSG_SIZE];
	...
	stBLOWFISH.stArgs.isEncrypt = 1;
	stBLOWFISH.stArgs.eAlgo = NCRYPT_ALGO_BLOWFISH;
	stBLOWFISH.stArgs.ePadd = NCRYPT_PADD_NULL;
	stBLOWFISH.stArgs.eMode = NCRYPT_MODE_ECB;
	stBLOWFISH.stArgs.pbKey = key32;
	stBLOWFISH.stArgs.szKey = ksize;
	stBLOWFISH.stArgs.pbInStream = sdata;
	stBLOWFISH.stArgs.szInStream = slen;
