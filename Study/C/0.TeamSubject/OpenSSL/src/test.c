...
int main(int argc, char **argv)
{
    ...
    unsigned char oubuf[2048];
    NCODE_TYPE_BASE64_ARGS args;
    
    memcpy(inbuf, &argv[1], strlen(argv[1]));
    memset(oubuf, 0x00, sizeof(oubuf));

    args.isEncode   = 1;
    args.pbInStream = inbuf;
    args.szInStream = strlen(inbuf);
    args.pbOutBuf   = oubuf;
    args.szOutBuf   = 2048;
    Ncode(NCODE_TYPE_BASE64, &args, sizeof(args));
