...
int serverWork() {
	int					MaxClients = FD_SETSIZE, MaxServers = 2, SzBuf = 512; /* FD_SETSIZE(64) */
	char				cliRecvBuf[512]; /* SzBuf */
	char				svrRecvBuf[512]; /* SzBuf */
	WSADATA				wsaData;
	SOCKET				tempCliSocket, tempSvrSocket, hostSocket, svrSocket[2], /* MaxServers */ cliSocket[64]; /* FD_SETSIZE */
	struct sockaddr_in	svrAddr, hostAddr, cliAddr;
	int					i, selSocCnt, byteLen, cliRecvLen, svrRecvLen, errCode, curSessionIdx = 0;
	fd_set				readFds;
				...
				else
				{
					...
					while (1)
					{
						...
						if (SOCKET_ERROR == (svrRecvLen = recv(tempSvrSocket, svrRecvBuf, SzBuf, 0)))
						{
							...
							{
								closesocket(tempSvrSocket);
								cliSocket[i] = 0;
								svrSocket[curSessionIdx] = 0;
