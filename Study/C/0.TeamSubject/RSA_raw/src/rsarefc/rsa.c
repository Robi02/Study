...
unsigned char *rnos;                          /* (R_RANDOM_STRUCT *randomStruct;) -> unsigned char *rnos *//* random structure */
{
  int status;
  unsigned char byte, pkcsBlock[MAX_RSA_MODULUS_LEN];
  unsigned int idx, modulusLen, pkcsRnoLen;
  ...
  idx = 0;
  pkcsBlock[idx] = 0; idx += 1;
  pkcsBlock[idx] = 2; idx += 1;
  
  pkcsRnoLen = modulusLen - inputLen - 3;
  R_memcpy ((POINTER)&pkcsBlock[idx], (POINTER)rnos, pkcsRnoLen); idx += pkcsRnoLen;
  
  pkcsBlock[idx] = 0; idx += 1;
  
  R_memcpy ((POINTER)&pkcsBlock[idx], (POINTER)input, inputLen); idx += inputLen;
