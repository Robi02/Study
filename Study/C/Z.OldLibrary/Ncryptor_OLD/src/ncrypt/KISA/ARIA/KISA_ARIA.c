...
#ifdef CT_BIG_ENDIAN
#undef CT_LITTLE_ENDIAN
#else
#ifndef CT_LITTLE_ENDIAN
...
/* abcd의 4 BYTE로 된 DWORD를 dcba로 변환하는 함수  */
#if defined(_MSC_VER)
...
/* BYTE array를 DWORD에 싣는 함수.  LITTLE_ENDIAN의 경우
 * 엔디안 변환 과정을 거친다. */
#ifdef CT_LITTLE_ENDIAN
#define DWORDLoad(ORIG, DEST) {			\
    DWORD ___t;					\
    BY(___t,0)=BY(ORIG,3);			\
    BY(___t,1)=BY(ORIG,2);			\
    BY(___t,2)=BY(ORIG,1);			\
...
static void Crypt(const BYTE *i, int Nr, const BYTE *rk, BYTE *o) {
  ...
  DWORDLoad(WO(i,0), t0); DWORDLoad(WO(i,1), t1);
  DWORDLoad(WO(i,2), t2); DWORDLoad(WO(i,3), t3);
...
static int EncKeySetup(const BYTE *mk, BYTE *rk, int keyBits) {
  ...
  DWORDLoad(WO(mk,0), w0[0]); DWORDLoad(WO(mk,1), w0[1]);
  DWORDLoad(WO(mk,2), w0[2]); DWORDLoad(WO(mk,3), w0[3]);
  ...
  FO;
  if (keyBits > 128) {
    DWORDLoad(WO(mk,4), w1[0]);
    DWORDLoad(WO(mk,5), w1[1]);
    if (keyBits > 192) {
      DWORDLoad(WO(mk,6), w1[2]);
      DWORDLoad(WO(mk,7), w1[3]);
    } else {
      w1[2]=w1[3]=0;
    }
  } else {
...
static int DecKeySetup(const BYTE *mk, BYTE *rk, int keyBits) {
  ...
  for (; a<z; a+=4, z-=4) {
    DWORDM1(a[0],t0); DWORDM1(a[1],t1); DWORDM1(a[2],t2); DWORDM1(a[3],t3);
    MM(t0,t1,t2,t3) P(t0,t1,t2,t3) MM(t0,t1,t2,t3)
      s0=t0; s1=t1; s2=t2; s3=t3;
    DWORDM1(z[0],t0); DWORDM1(z[1],t1); DWORDM1(z[2],t2); DWORDM1(z[3],t3);
    MM(t0,t1,t2,t3) P(t0,t1,t2,t3) MM(t0,t1,t2,t3)
...
void ARIA_KeySched(BYTE *UserKey, DWORD UserKeyLen, BYTE *RoundKey, int isDecKey)
{
    if ( isDecKey == 1 )
    {
...
void ARIA_Encrypt(BYTE *RoundKey, BYTE *Data, int szData)
{
    BYTE rtBuf[szData];

    Crypt(Data, 12, RoundKey, rtBuf);
    memcpy(Data, rtBuf, sizeof(rtBuf));
}
                                    
void ARIA_Decrypt(BYTE *RoundKey, BYTE *Data, int szData)
{
    BYTE rtBuf[szData];
